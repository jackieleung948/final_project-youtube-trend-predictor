...
import numpy as np
import os
from config import (TRAIN_END, STEP_BUCKETS, HORIZON_BUCKETS, LABELS_CSV)
...
def rolling_windows(n_train, n_total, HORIZON_BUCKETS, STEP_BUCKETS):
    """
    ...
    - HORIZON_BUCKETS: int, forecast horizon in buckets
    - STEP_BUCKETS: int, step size for rolling windows in buckets

    Returns:
    - windows: list of tuples, each containing (train_start, train_end, test_start, test_end)
    """
    windows, origin = [], n_train
    while origin + HORIZON_BUCKETS <= n_total:
        windows.append((0, origin, origin, origin + HORIZON_BUCKETS))
        origin += STEP_BUCKETS
    return windows
...
def run(models):
    ...
    for keyword, ser in series.items():
        ...
        fitted_lstm = fit_lstm(train_ser.values) if (
            "lstm" in models or "hybrid_avg" in models) else None
        fitted_residual = fit_residual_lstm(
            train_ser.values) if "hybrid_residual" in models else None
        train_min, train_max, train_mean = train_ser.min(), train_ser.max(), train_ser.mean()
        n_train, n_total = len(train_ser), len(ser)
        windows = rolling_windows(
            n_train, n_total, HORIZON_BUCKETS, STEP_BUCKETS)
        for train_start, train_end, test_start, test_end in windows:
            train_data = ser.iloc[train_start:train_end]
            test_data = ser.iloc[test_start:test_end]
            arima_fc = lstm_fc = None
            need_components = any(model in models for model in [
                                  "arima", "lstm", "hybrid_avg"])
            arima_fc = forecast_arima(
                train_data, HORIZON_BUCKETS) if need_components else None
            lstm_fc = forecast_lstm(
                fitted_lstm, train_data.values, HORIZON_BUCKETS) if need_components else None
            for model_name in models:
                if model_name == "arima":
                    ...
                    forecast = None if (
                        arima_fc is None or lstm_fc is None) else 0.5 * arima_fc + 0.5 * lstm_fc
                elif model_name == "hybrid_residual":
                    forecast = forecast_residual_hybrid(
                        fitted_residual, train_data.values, HORIZON_BUCKETS)
                elif model_name in MODEL_FUNCTIONS:
                    forecast = MODEL_FUNCTIONS[model_name](
                        train_data, HORIZON_BUCKETS)
                else:
                    print(f"Model {model_name} not recognized. Skipping")
                    continue
                if forecast is None or len(forecast) != len(test_data):
                    failures[model_name] = failures.get(model_name, 0) + 1
                    print(
                        f"Warning: Forecast for model {model_name} on keyword {keyword} has length "
                        f"{len(forecast) if forecast is not None else 0} but expected {len(test_data)}. Skipping this evaluation."
                    )
                    continue
    ...
    if not results_df.empty and len(models) > 1:
        counts = results_df.groupby(["keyword", "test_start"])[
            "model"].nunique()
        complete = counts[counts == len(models)].index
        before = len(results_df)
        results_df = results_df.set_index(
            ["keyword", "test_start"]).loc[complete].reset_index()
        after = len(results_df)
        print(
            f"Common set restriction: {before} -> {after} rows (removed {before - after} rows)")
...
def evaluate_labels(res):
    ...
    pred = (res.groupby(["model", "keyword"])["pred_trending"].any(
    ...
    for threshold in ["trending_25", "trending_50"]:
        ...
        for model, g in merged.groupby("model"):
            ...
            f1 = 2 * (precision * recall) / (precision +
                                             recall) if (precision + recall) > 0 else np.nan
            print(
                f"Model: {model}, Precision: {precision:.3f}, Recall: {recall:.3f}, F1: {f1:.3f}, TP: {tp}, FP: {fp}, FN: {fn}")


if __name__ == "__main__":
    ap = argparse.ArgumentParser(
        description="Evaluate forecasting models on YouTube trend data.")
    ap.add_argument("--models", nargs="+", required=True,
                    help="List of models to evaluate (e.g., arima naive_last naive_mean)")
    args = ap.parse_args()
