...
def main():
    ...
    # Temporary: Filtered to keywords collected after 2026-07-17 18:00:00 to validate keyword filtering approach for backtesting (LSTM)
    ...
    print(
            f"Filtered to {len(valid_keywords)} keywords with at least 10 hour buckets for backtesting (LSTM).")
    ...
    # Limit to top 50 keywords by total frequency for backtesting (LSTM)
    top_keywords = df.groupby("keyword")["frequency"].sum().nlargest(50).index
    df = df[df["keyword"].isin(top_keywords)]
    print("Filtered to top 50 keywords by total frequency for backtesting (LSTM):")

    # For each keyword, split the data into training and testing sets for backtesting LSTM model
    backtest_results = []
