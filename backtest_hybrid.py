...
def main():
    ...
    # Temporary: Filtered to keywords collected after 2026-07-17 18:00:00 to validate keyword filtering approach for backtesting (Hybrid)
    ...
    print(
            f"Filtered to {len(valid_keywords)} keywords with at least 10 hour buckets for backtesting (Hybrid).")
    ...
    # Limit to top 50 keywords by total frequency for backtesting (Hybrid)
    top_keywords = df.groupby("keyword")["frequency"].sum().nlargest(50).index
    df = df[df["keyword"].isin(top_keywords)]
    print("Filtered to top 50 keywords by total frequency for backtesting (Hybrid):")

    # For each keyword, split the data into training and testing sets for backtesting Hybrid model
    backtest_results = []
    for keyword in df["keyword"].unique():
        ...
        try:
            ...
            input_seq = train_scaled[-n_steps:].tolist()
            lstm_forecast = []
            for _ in range(6):
            ...
            hybrid_forecast = 0.5 * arima_forecast + 0.5 * lstm_forecast
            
            mae = abs(hybrid_forecast - test_scaled).mean()
            backtest_results.append({"keyword": keyword, "mae": mae})
            print(f"Keyword: {keyword}, MAE: {mae:.4f}")
        except Exception as e:
            print(f"Hybrid failed for keyword '{keyword}': {e}")
            continue
